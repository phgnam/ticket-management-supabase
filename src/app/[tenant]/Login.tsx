...
export const Login = ({ formType = "pw-login", tenant, tenantName }: any) => {
  ...
  const getPath = (subPath?: any) => urlPath(subPath ?? "", tenant);
  ...
  useEffect(() => {
    const {
      ...
      if (event === "SIGNED_IN") {
        if (session && session.user.app_metadata.tenants?.includes(tenant)) {
